...
function collectionForEntityType(entityType) {
  ...
  if (entityType === 'guide') return 'guides';
...
async function assertApplySafety({ root, releaseCandidateId, plan }) {
  ...
  if ((release.publication_mode || 'legacy') !== 'legacy') {
    throw new Error('This release uses reviewed GitHub PR publication and cannot write live JSON directly.');
  }
...
export async function recoverPartialLiveApply({
  ...
  const pool = await getDbPool({ requireConfigured: true });
  const [releaseRowsResult] = await pool.execute(
    'SELECT publication_mode FROM release_candidates WHERE id = ? LIMIT 1',
    [releaseCandidateId]
  );
  if (!releaseRowsResult[0]) throw new Error(`Release candidate not found: ${releaseCandidateId}`);
  if ((releaseRowsResult[0].publication_mode || 'legacy') !== 'legacy') {
    throw new Error('This release uses reviewed GitHub PR publication and cannot enter live-JSON recovery.');
  }
