...
import { AUDIT_ELECTIVE_MIN_NOTE_LENGTH, isAuditStylePromotionSubject } from './verification-review.js';
import {
  ELECTIVE_OPTION_REVIEW_WARNING_CODE,
  STALE_DRAFT_COPY_WARNING_CODE,
  hasElectiveOptionAmbiguity,
  staleDraftCopyFindings,
...
function promotionMetadataForItem(row) {
  ...
  return {
    ...
    workflow: diff?.workflow || null,
    public_subject: replacement || null,
...
function itemSummaryFromRow(row) {
  ...
  return {
    ...
    workflow: promotionMetadata.workflow,
    public_subject: promotionMetadata.public_subject,
...
function buildWarnings(items) {
  ...
  for (const item of items) {
    ...
    if (item.proposal_status !== 'approved_for_draft') {
      addWarning(warnings, {
        code: 'proposal_not_approved_for_draft',
        message: `Release contains proposals that are no longer approved_for_draft. Regenerate release artifacts after proposal repair. Proposal ${item.proposal_id} status is ${item.proposal_status}.`,
        itemId: item.item_id,
    ...
    if (['verified_promotion', 'verified_amendment'].includes(item.workflow_type)) {
      const source = item.public_source || {};
      const staleFindings = staleDraftCopyFindings(item.public_subject || {});
      if (staleFindings.length) {
        addWarning(warnings, {
          code: STALE_DRAFT_COPY_WARNING_CODE,
          message: `Verified promotion proposal ${item.proposal_id} contains draft-only wording in public payload fields: ${staleFindings.map(finding => `${finding.path} (${finding.phrase})`).join(', ')}.`,
          itemId: item.item_id,
          proposalId: item.proposal_id,
          entityKey: item.entity_key,
          file: item.diff.target_file,
      ...
      const reviewerNote = String(item.workflow?.reviewer_note || '').trim();
      if (
        hasElectiveOptionAmbiguity(item.public_subject)
        && (item.workflow?.elective_option_confirmed !== true || reviewerNote.length < AUDIT_ELECTIVE_MIN_NOTE_LENGTH)
      ) {
        addWarning(warnings, {
          code: ELECTIVE_OPTION_REVIEW_WARNING_CODE,
          message: `Verified promotion proposal ${item.proposal_id} has OR/elective-option ambiguity. Reviewer must explicitly confirm this is a standalone elective page (and record a brief reason of at least ${AUDIT_ELECTIVE_MIN_NOTE_LENGTH} characters) so public copy does not imply the course is mandatory for every student.`,
          itemId: item.item_id,
          proposalId: item.proposal_id,
          entityKey: item.entity_key,
          file: item.diff.target_file,
      ...
      if (
        isAuditStylePromotionSubject(item.public_subject)
        && (item.workflow?.audit_course_publication_confirmed !== true || reviewerNote.length < AUDIT_ELECTIVE_MIN_NOTE_LENGTH)
      ) {
        addWarning(warnings, {
          code: 'audit_course_publication_review_required',
          message: `Verified promotion proposal ${item.proposal_id} is for a mandatory non-credit, audit, internship, project, or zero-credit row. Verified course existence does not automatically mean the page is useful enough to publish; require an explicit reviewer publication confirmation and a brief reason of at least ${AUDIT_ELECTIVE_MIN_NOTE_LENGTH} characters.`,
          itemId: item.item_id,
          proposalId: item.proposal_id,
          entityKey: item.entity_key,
          file: item.diff.target_file,
...
export async function generateReleaseReviewSummary({ releaseCandidateId, actor = null, auditEvents = false, db = null }) {
  const pool = db || await getDbPool({ requireConfigured: true });
