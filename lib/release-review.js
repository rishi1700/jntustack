...
function addWarning(warnings, { code, message, itemId = null, proposalId = null, entityKey = null, file = null }) {
  warnings.push({
    code,
    severity: 'blocking',
    blocking: true,
...
function buildWarnings(items) {
  ...
  for (const item of items) {
    ...
    if (item.export_validation_status !== 'passed') {
      addWarning(warnings, {
        code: 'failed_validation',
        message: item.proposal_export_id
          ? `Export ${item.proposal_export_id} validation is ${item.export_validation_status}.`
          : `Proposal ${item.proposal_id} has no export.`,
        itemId: item.item_id,
        proposalId: item.proposal_id,
        entityKey: item.entity_key,
      });
  ...
  for (const [file, count] of fileCounts) {
    if (count > 1) {
      addWarning(warnings, {
        ...
        message: `${file} is touched by ${count} proposals.`,
        file,
      });
    }
  }
...
export async function generateReleaseReviewSummary({ releaseCandidateId, actor = null, auditEvents = false }) {
  ...
  const summary = {
    ...
    warnings,
    blocking_warning_count: warnings.filter(warning => warning.blocking).length,
    has_blocking_warnings: warnings.some(warning => warning.blocking),
    not_published: true,
  };
