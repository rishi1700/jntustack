...
function buildWarnings(items) {
  ...
  for (const item of items) {
    ...
    if (item.workflow_type === 'verified_promotion') {
      ...
      if (hasElectiveOptionAmbiguity(item.public_subject, item.workflow) && !hasElectiveOptionReviewNote(item.workflow?.reviewer_note)) {
        addWarning(warnings, {
          code: ELECTIVE_OPTION_REVIEW_WARNING_CODE,
          message: `Verified promotion proposal ${item.proposal_id} has OR/elective-option ambiguity. Reviewer note must explain whether this is a standalone elective subject page, whether the title should be published as an elective option, and that public page copy avoids implying the course is mandatory for every student.`,
          itemId: item.item_id,
          proposalId: item.proposal_id,
          entityKey: item.entity_key,
          file: item.diff.target_file,
      ...
      if (
        ...
        && (
          String(item.workflow?.audit_course_publication_confirmation || '').trim() !== AUDIT_COURSE_PUBLICATION_CONFIRMATION
          || String(item.workflow?.reviewer_note || '').trim().length < 40
        )
      ) {
        addWarning(warnings, {
          code: 'audit_course_publication_review_required',
          message: `Verified promotion proposal ${item.proposal_id} is for a mandatory non-credit, audit, internship, project, or zero-credit row. Verified course existence does not automatically mean the page is useful enough to publish; require an explicit reviewer decision with "${AUDIT_COURSE_PUBLICATION_CONFIRMATION}" and a reviewer note explaining why this page should be public.`,
          itemId: item.item_id,
          proposalId: item.proposal_id,
          entityKey: item.entity_key,
          file: item.diff.target_file,
