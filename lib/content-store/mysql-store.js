...
function subjectFilesFromContent(content) {
  return [...new Set(content.subjects.map(subject => {
    if (subject.offerings?.some(offering => offering.year === 1) || subject.year === 1) return 'subjects-first-year.json';
    const idBranch = String(subject.id || '').match(/^r\d+-([a-z]+)-/)?.[1];
    return `subjects-${String(subject.branch || idBranch || subject.branchCodes?.[0] || 'unknown').toLowerCase()}.json`;
  }))].sort();
}
