...
function adminSecret(config) {
  // No fallback: signing sessions with the password hash (or worse, the
  // plaintext password) would mean anyone who ever sees that value can also
  // forge session cookies. A dedicated ADMIN_SESSION_SECRET is required --
  // its absence makes adminIsConfigured() false, which makes every admin
  // route refuse with a 503 config-error page (see requireAdmin/GET login
  // in routes/admin.js) instead of quietly signing with a weaker secret.
...
export function createAdminCsrfToken(cookieValue, action, config = getAdminConfig()) {
  const secret = adminSecret(config);
  const normalizedAction = String(action || '');
  if (!secret || !normalizedAction || !verifyAdminCookie(cookieValue, config)) return '';
  return hmac(`csrf:v1:${normalizedAction}:${cookieValue}`, secret);
}

export function verifyAdminCsrfToken(token, cookieValue, action, config = getAdminConfig()) {
  if (!token || !cookieValue) return false;
  const expected = createAdminCsrfToken(cookieValue, action, config);
...
export function passwordHashHelp() {
  return 'Use ADMIN_PASSWORD_HASH=pbkdf2:<iterations>:<salt>:<base64url hash> (recommended). sha256:<hex> is still accepted but deprecated -- unsalted and single-round, weak if ever leaked; migrate to pbkdf2:. ADMIN_PASSWORD is accepted for local setup only.';
}
