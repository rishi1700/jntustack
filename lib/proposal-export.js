...
import { getAsset, getAssetFileStatus } from './assets.js';
...
function subjectIdentityMatches(subject, entityKey, payload) {
  const expected = [entityKey, payload?.id, payload?.seo?.slug, payload?.subject_code]
    .map(normalize)
    .filter(Boolean);
  return [subject?.id, subject?.seo?.slug, subject?.subject_code]
    .map(normalize)
    .filter(Boolean)
    .some(candidate => expected.includes(candidate));
}

function inferredSubjectFilename(payload) {
  if (payload?.offerings?.some(offering => offering?.year === 1) || payload?.year === 1) {
    return 'subjects-first-year.json';
  }
  const idBranch = String(payload?.id || '').match(/^r\d+-([a-z]+)-/)?.[1];
  const branch = normalize(payload?.branch || idBranch || payload?.branchCodes?.[0] || 'unknown');
  return `subjects-${branch}.json`;
}

export async function resolveSubjectTargetForExport({ root, entityKey, payload, existing = false }) {
  if (!existing) {
    return { dataFileHint: `data/${inferredSubjectFilename(payload)}`, index: -1 };
  }
  const dataDir = path.join(root, 'data');
  const filenames = (await fs.readdir(dataDir))
    .filter(filename => /^subjects-[a-z0-9-]+\.json$/i.test(filename))
    .sort();
  const matches = [];
  for (const filename of filenames) {
    const document = JSON.parse(await fs.readFile(path.join(dataDir, filename), 'utf8'));
    const index = (document.subjects || []).findIndex(subject => subjectIdentityMatches(subject, entityKey, payload));
    if (index >= 0) matches.push({ dataFileHint: `data/${filename}`, index });
  }
  if (matches.length !== 1) {
    throw new Error(
      matches.length
        ? `Subject export target is ambiguous across ${matches.map(match => match.dataFileHint).join(', ')}.`
        : 'Existing subject export target was not found in any data/subjects-*.json file.'
    );
  }
  return matches[0];
}
...
export async function exportProposalForReview({ root, proposalId, actor = null }) {
  ...
  try {
    ...
    const verifiedPromotion = isVerifiedReviewDiff(proposal.diff);
    if (proposal.diff?.workflow?.type === 'verified_amendment') {
      const sourceAsset = await getAsset(proposal.sourceAsset?.id);
      if (!sourceAsset) throw new Error('Verified provenance amendment source asset is missing.');
      ...
      const prepared = prepareVerifiedProvenanceAmendment({
        ...
        proposedPayload: sourcePayload,
        sourceAsset,
        sourceAssetFileStatus,
        discoverySource,
        reviewerNote: proposal.diff.workflow.reviewer_note,
        confirmationPhrase: VERIFIED_AMENDMENT_CONFIRMATION,
      });
      if (prepared.diff.workflow.source_asset_sha256 !== proposal.diff.workflow.source_asset_sha256) {
        throw new Error('Verified provenance amendment source asset checksum no longer matches its reviewed proposal.');
      }
    }
    ...
    if (proposal.entityType === 'subject') {
      const resolvedTarget = await resolveSubjectTargetForExport({
        root,
        entityKey: proposal.entityKey,
        payload: validation.normalizedPayload,
        existing: target.index >= 0,
      });
      target.dataFileHint = resolvedTarget.dataFileHint;
      target.index = resolvedTarget.index;
    }
    ...
    const exportPayload = {
      ...
      notes: verifiedPromotion ? [
        ...
        'Verified review proposals must continue through release candidate, apply plan, and final live apply.',
