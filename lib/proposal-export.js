...
function findTarget(content, entityType, entityKey, payload) {
  ...
  if (entityType === 'subject') {
    const index = (content.data.subjects || []).findIndex(subject => [
      ...
      payload?.id,
      payload?.seo?.slug,
    ].some(candidate => normalize(candidate) === key || (candidate && normalize(candidate) === normalize(payload?.id))));
    return {
      collection: 'subjects',
  ...
  if (entityType === 'college') {
    ...
    ].some(candidate => normalize(candidate) === key || (candidate && normalize(candidate) === normalize(payload?.name))));
    return {
      collection: 'colleges',
  ...
  const index = (content.branchProfiles || []).findIndex(profile => [
    ...
    profile.tagline,
    payload?.branch,
  ].some(candidate => normalize(candidate) === key || (candidate && normalize(candidate) === normalize(payload?.branch))));
  return {
