  ...
  for (const subject of subjects) {
    ...
    if (group.examples.length < 3) {
      ...
      group.examples.push({
        ...
        branch: subject?.branch || subject?.branchCodes?.join('/') || firstOffering?.branchCodes?.join('/') || '',
        semester: subject?.year_sem_label || (firstOffering ? `${firstOffering.year}-${firstOffering.semester}` : ''),
      });
    }
  }
