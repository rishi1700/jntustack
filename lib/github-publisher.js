...
export async function recordGitHubPublicationAudit(
  db,
  { actor, action, entityId, before = null, after = null }
) {
  await db.execute(
...
async function mutatePublicationWithAudit(db, {
  ...
  try {
    ...
    await recordGitHubPublicationAudit(db, {
      actor,
      action,
...
export class GitHubPublisher {
  constructor({
    ...
    root = process.cwd(),
    env = process.env,
    siteUrl = env.SITE_URL || 'https://jntustack.com',
    fetchImpl = globalThis.fetch,
    defaultBranch = env.GITHUB_DEFAULT_BRANCH || 'main',
    deploymentVerificationOptions = {},
    signingPrivateKey = env.PUBLICATION_SIGNING_PRIVATE_KEY_BASE64
      || env.PUBLICATION_SIGNING_PRIVATE_KEY,
    signingKeyId = env.PUBLICATION_SIGNING_KEY_ID,
    trustReady = getGitHubPublicationTrustReady(env),
    storageReadiness = getAssetStoragePublicationReadiness(env),
    storagePersistenceCheck = checkAssetStoragePersistence,
  } = {}) {
    this.db = db;
    this.github = github;
    ...
    this.storagePersistenceCheck = storagePersistenceCheck;
  }
  ...
  async createPublication({ releaseCandidateId, actor = null, confirmation = '' }) {
    ...
    if (!this.storageReadiness?.ready) {
      throw new PublicationStateError(
        'GitHub publication is fail-closed until durable asset storage is configured and its persistence is verified.',
        'asset_storage_not_ready'
      );
    }
    let persistedStorage;
    try {
      persistedStorage = await this.storagePersistenceCheck({
        root: this.root,
        env: this.env,
        seedIfMissing: false,
      });
    ...
    if (!persistedStorage?.ready) {
      throw new PublicationStateError(
        'GitHub publication is fail-closed until the configured asset store has survived a commit-changing deployment.',
        'asset_storage_persistence_not_verified'
      );
    }
