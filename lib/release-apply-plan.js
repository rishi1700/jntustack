...
function safeDataFileHint(filePath) {
  ...
  if (
    ...
    || ['data/release.json', 'data/release-artifact.json'].includes(value)
  ) {
    throw new Error(`Unsafe release apply target: ${filePath || '(missing)'}.`);
  }
  return value;
}
...
function entityMatches(collection, left, right) {
  ...
  if (collection === 'guides') return guideMatches(left, right);
  return false;
}
...
async function storeReleaseApplyPlan(pool, {
  ...
  if (existing[0]) {
    ...
    if (Number(updateResult.affectedRows) !== 1) {
      throw new Error('This apply plan was sealed while regeneration was running. Prepare a fresh release candidate.');
    }
  } else {
    await pool.execute(
...
async function withCurrentReviewWarnings({ plan, releaseCandidateId }) {
  const reviewSummary = await generateReleaseReviewSummary({ releaseCandidateId });
  return {
    ...plan,
    final_warnings: reviewSummary.blocking_warnings || reviewSummary.warnings || [],
    informational_warnings: reviewSummary.informational_warnings || [],
    validation_summary: reviewSummary.validation_status_per_item || plan.validation_summary || [],
    current_review_status: {
      blocking_warning_count: reviewSummary.blocking_warning_count,
      informational_warning_count: reviewSummary.informational_warning_count,
      generated_from_current_policy: true,
    },
  };
}
...
export async function generateReleaseApplyPlan({ root = process.cwd(), releaseCandidateId, actor = null } = {}) {
  const database = await getDbPool({ requireConfigured: true });
  ...
  try {
    ...
    const [sealedRows] = await pool.execute(
    ...
    if (sealedRows[0]?.artifact_hash) {
      const message = 'This apply plan is already sealed for GitHub publication and cannot be regenerated. Prepare a fresh release candidate.';
      await audit(pool, {
        actor,
        action: 'release_apply_plan.regeneration_blocked',
        entityType: 'release_candidate',
        entityId: releaseCandidateId,
        after: { reason: message, artifact_hash: sealedRows[0].artifact_hash },
      });
      throw new Error(message);
    }
    const reviewSummary = await generateReleaseReviewSummary({ releaseCandidateId, db: pool });
    ...
    for (const row of itemRows) {
      ...
      let targetFile;
      try {
        targetFile = safeDataFileHint(exportPayload?.target?.data_file_hint);
      } catch {
    ...
    const finalWarnings = reviewSummary.blocking_warnings || reviewSummary.warnings || [];
    const informationalWarnings = reviewSummary.informational_warnings || [];
...
async function reconstructReleaseApplyPlanFromMetadata(pool, { root, releaseCandidateId, actor = null } = {}) {
  ...
  for (const row of rows.itemRows) {
    ...
    let file;
    try {
      file = safeDataFileHint(target.data_file_hint);
    } catch {
      continue;
    }
    if (!patch) continue;
...
export async function getReleaseApplyPlan({ root = process.cwd(), releaseCandidateId } = {}) {
  ...
  if (stored) return withCurrentReviewWarnings({ plan: stored, releaseCandidateId });
  ...
  if (tmpPlan) {
    ...
    return withCurrentReviewWarnings({ releaseCandidateId, plan: {
      ...tmpPlan,
      storage: releasePlanStorage({
