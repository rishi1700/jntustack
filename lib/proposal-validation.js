...
function getValidators(root) {
  ...
  validatorCache = {
    ...
    guide: ajv.getSchema(`${schema.$id}#/definitions/Guide`),
  };
...
export function normalizeEntityKey(entityType, entityKey) {
  ...
  if (!key) return '';
  if (entityType === 'subject' || entityType === 'guide') return slugify(key);
  ...
  if (entityType === 'guide') {
    ...
    if (Array.isArray(normalized.sections)) {
      normalized.sections = normalized.sections.map(section => (
        section && typeof section === 'object' && section.id
          ? { ...section, id: slugify(section.id) }
          : section
      ));
    }
  }
...
function workflowErrors(entityType, payload, { allowVerifiedSource = false } = {}) {
  ...
  if (entityType === 'subject' && allowVerifiedSource && payload.source?.status === 'verified') {
    for (const finding of staleDraftCopyFindings(payload)) {
      errors.push({
        path: finding.path,
        message: `Verified promotion payload contains draft-only wording (${finding.phrase}). Remove stale draft copy before public promotion.`,
        keyword: STALE_DRAFT_COPY_WARNING_CODE,
        params: finding,
      });
    }
  }
