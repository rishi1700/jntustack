...
function normalizeSource(payload) {
  ...
  if (!source.status || source.status === 'verified') source.status = SAFE_SOURCE_STATUS;
  next.source = source;
  return next;
}
...
export function normalizeProposalPayload(entityType, payload) {
  let normalized = deepNormalize(parseJson(payload, {}));
  if (!normalized || typeof normalized !== 'object' || Array.isArray(normalized)) return normalized;

  normalized = normalizeSource(normalized);
...
export function validateProposalPayload({ root, entityType, payload }) {
  ...
  const normalizedPayload = normalizeProposalPayload(entityType, payload);
  const validate = getValidators(root)[entityType];
  const valid = validate(normalizedPayload);
  const errors = [
    ...ajvErrors(validate),
    ...workflowErrors(entityType, normalizedPayload),
  ];
