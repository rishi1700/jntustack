...
export async function reviewContentProposal({ id, action, note = '', actor = null }) {
  ...
  if ((action === 'request_changes' || config.requireNote) && !note.trim()) {
    throw new Error(action === 'approve_for_draft'
      ? 'A reviewer note is required when approving for draft preparation.'
      : 'A reviewer note is required when requesting changes.');
  }
