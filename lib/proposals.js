...
import { isVerifiedReviewDiff } from './verified-amendments.js';
...
export async function getContentProposal(id) {
  ...
  const sourceAssetId = Number(proposal.diff?.workflow?.source_asset_id || 0);
  if (sourceAssetId > 0) {
    const [assetRows] = await pool.execute(
      `SELECT sa.id, sa.source_url, sa.resolved_url, sa.sha256_checksum,
        sa.download_status, sa.storage_provider, sa.discovery_source_id,
    ...
    if (asset) {
      proposal.sourceAsset = {
        id: asset.id,
        sourceUrl: asset.source_url,
        resolvedUrl: asset.resolved_url,
        sha256Checksum: asset.sha256_checksum,
        downloadStatus: asset.download_status,
        storageProvider: asset.storage_provider,
        discoverySourceId: asset.discovery_source_id,
        discoverySourceName: asset.discovery_source_name,
      };
    }
  }
...
export async function createContentProposal({
  ...
  const validation = validateProposalPayload({ root, entityType, payload: proposedPayload, allowVerifiedSource });
  const pool = await getDbPool({ requireConfigured: true });
...
export async function validateContentProposal({ root = process.cwd(), id, actor = null }) {
  ...
  try {
    ...
    const validation = validateProposalPayload({
      ...
      allowVerifiedSource: isVerifiedReviewDiff(before.diff_json),
    });
