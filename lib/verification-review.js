...
export const AUDIT_COURSE_PUBLICATION_CONFIRMATION = 'PUBLISH AUDIT COURSE';
...
export function validatePromotionReview({
  ...
  reviewerNote = '',
  confirmationPhrase = '',
  auditCoursePublicationConfirmation = '',
}) {
  const errors = [];
  ...
  if (
    isAuditStylePromotionSubject(subject)
    && String(auditCoursePublicationConfirmation || '').trim() !== AUDIT_COURSE_PUBLICATION_CONFIRMATION
  ) {
    errors.push(`audit_course_publication_review_required: Mandatory non-credit, audit, internship, project, and zero-credit rows require explicit public-publication confirmation: ${AUDIT_COURSE_PUBLICATION_CONFIRMATION}.`);
  }
  if (isAuditStylePromotionSubject(subject) && String(reviewerNote || '').trim().length < 40) {
    errors.push('audit_course_publication_review_required: reviewer note must explain why this content-light audit/non-credit page should be public.');
  }
  if (hasElectiveOptionAmbiguity(subject, { reviewer_note: reviewerNote }) && !hasElectiveOptionReviewNote(reviewerNote)) {
    errors.push(`${ELECTIVE_OPTION_REVIEW_WARNING_CODE}: elective-option or OR wording requires a reviewer note explaining whether this is a standalone elective subject page, whether the title should be published as an elective option, and that public copy avoids implying the course is mandatory for every student.`);
  }
  ...
  try {
    ...
    const diff = {
      ...
      workflow: {
        ...
        audit_course_publication_confirmation: String(auditCoursePublicationConfirmation || '').trim() || null,
