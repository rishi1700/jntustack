...
import {
  ELECTIVE_OPTION_REVIEW_WARNING_CODE,
  hasElectiveOptionAmbiguity,
  removeStaleVerifiedPromotionNotes,
} from './verified-promotion-guardrails.js';

export const PROMOTE_TO_VERIFIED_CONFIRMATION = 'PROMOTE TO VERIFIED';

// Audit-style and elective-option promotions require an explicit checkbox
// confirmation AND a brief free-text reason. The reason is intentionally NOT
// keyword-matched (that was friction without real safety value) -- we only
// require enough length to record why the reviewer chose to publish.
export const AUDIT_ELECTIVE_MIN_NOTE_LENGTH = 30;
...
function isZeroCredit(subject) {
  const value = creditValue(subject);
  if (value === null || value === undefined || value === '') return false;
  return Number(value) === 0;
}

export function isAuditStylePromotionSubject(subject = {}) {
...
export function buildVerifiedPromotionPayload(subject) {
  const payload = removeStaleVerifiedPromotionNotes(clone(subject));
...
export function validatePromotionReview({
  ...
  reviewerNote = '',
  confirmationPhrase = '',
  auditCoursePublicationConfirmed = false,
  electiveOptionConfirmed = false,
}) {
  const errors = [];
  ...
  if (!subject.source?.retrieved_date || !String(subject.source.retrieved_date).trim()) {
    errors.push('missing_source_retrieved_date: source.retrieved_date is required before verified promotion.');
  }
  if (!subject.source?.college_source_note || !String(subject.source.college_source_note).trim()) {
    errors.push('missing_public_source_caveat: source.college_source_note is required before verified promotion.');
  }
  ...
  const trimmedReviewerNote = String(reviewerNote || '').trim();
  if (isAuditStylePromotionSubject(subject)) {
    if (!auditCoursePublicationConfirmed) {
      errors.push('audit_course_publication_review_required: Mandatory non-credit, audit, internship, project, and zero-credit rows require an explicit publication confirmation before they can go public.');
    } else if (trimmedReviewerNote.length < AUDIT_ELECTIVE_MIN_NOTE_LENGTH) {
      errors.push(`audit_course_publication_review_required: Add a brief reviewer note (at least ${AUDIT_ELECTIVE_MIN_NOTE_LENGTH} characters) recording why this content-light page is worth publishing.`);
    }
  }
  if (hasElectiveOptionAmbiguity(subject)) {
    if (!electiveOptionConfirmed) {
      errors.push(`${ELECTIVE_OPTION_REVIEW_WARNING_CODE}: elective-option or OR wording requires an explicit confirmation that this is a standalone elective page and public copy does not imply the course is mandatory for every student.`);
    } else if (trimmedReviewerNote.length < AUDIT_ELECTIVE_MIN_NOTE_LENGTH) {
      errors.push(`${ELECTIVE_OPTION_REVIEW_WARNING_CODE}: Add a brief reviewer note (at least ${AUDIT_ELECTIVE_MIN_NOTE_LENGTH} characters) recording why this elective page is safe to publish.`);
    }
  }
...
export async function createVerifiedPromotionProposal({
  ...
  const review = validatePromotionReview({
    ...
    reviewerNote,
    confirmationPhrase,
    auditCoursePublicationConfirmed,
    electiveOptionConfirmed,
  });
  ...
  try {
    ...
    const diff = {
      ...
      workflow: {
        ...
        audit_course_publication_confirmed: Boolean(auditCoursePublicationConfirmed),
        audit_course_publication_required: isAuditStylePromotionSubject(subject),
        elective_option_confirmed: Boolean(electiveOptionConfirmed),
        elective_option_ambiguous: hasElectiveOptionAmbiguity(subject),
      },
    };
