...
function parseSemesterHeading(line) {
  ...
  const match = text.match(/\b(?:B\.?\s*Tech\.?\s*)?[-. ]*([IV]{1,3}|1st|2nd|3rd|4th|[1-4])\s+Year\s+([IV]{1,2}|1st|2nd|[12])\s+Semester\b/i);
...
function isCourseStructureHeader(line) {
  return /\bS\.?\s*No\.?\b/i.test(line)
    && /\bCategory\b/i.test(line)
    && /\bTitle\b/i.test(line)
    && /\bCredits?\b/i.test(line);
}
...
function parseTirumalaCourseRows({ pageTexts, asset, parserVersion }) {
  ...
  pageTexts.forEach((pageText, pageIndex) => {
    ...
    for (const rawLine of pageText.split(/\n+/)) {
      ...
      if (isCourseStructureHeader(line)) {
        finishRow();
        inCourseTable = true;
        continue;
      }
