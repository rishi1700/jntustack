...
function romanToNumber(value) {
  ...
  if (text === 'V') return 5;
  if (text === 'VI') return 6;
  if (text === 'VII') return 7;
  if (text === 'VIII') return 8;
  return undefined;
}
...
function semesterNumberToYearSem(semesterNumber) {
  if (!Number.isInteger(semesterNumber) || semesterNumber < 1 || semesterNumber > 8) return null;
...
function parseLbrceSemesterHeading(line) {
  const text = normalizePdfLine(line);
  const match = text.match(/\b(I|II|III|IV|V|VI|VII|VIII)\s*-?\s*SEMESTER\b/i);
  if (!match) return null;
  const semesterNumber = romanToNumber(match[1]);
  const yearSem = semesterNumberToYearSem(semesterNumber);
...
function isLbrceCourseStructureHeader(line) {
  const text = normalizePdfLine(line);
  return /\bS\.?\s*N\.?\s*o\.?\b/i.test(text)
    && /\bCourse\b/i.test(text)
    && /\bcode\b/i.test(text)
    && /\bCourse\s+Title\b/i.test(text)
    && /\bL\b/i.test(text)
    && /\bT\b/i.test(text)
    && /\bP\b/i.test(text)
    && /\bCIE\b/i.test(text)
...
function lbrceSectionFromLine(line) {
  const text = normalizePdfLine(line);
  if (/^Theory Courses$/i.test(text)) {
    return { label: 'Theory Courses', type_hint: 'theory' };
  }
  if (/^Laboratory Courses$/i.test(text)) {
    return { label: 'Laboratory Courses', type_hint: 'lab' };
  }
  return null;
}

function isLbrceCourseRowStart(line) {
  const text = normalizePdfLine(line);
  return /^\d{1,2}$/.test(text)
    || /^\d{1,2}\s+(?:[A-Z0-9]{4,12}|Program\s+Elective|Open\s+Elective|Minor\s+Course|Honou?rs?\s+Course|[A-Za-z])\b/i.test(text);
}

function isLbrceStandaloneCourseCodeLine(line) {
  const text = normalizePdfLine(line);
  return /^(?=[A-Z0-9]*\d)[A-Z0-9]{4,12}\b/i.test(text);
}

function lbrceRowHasTerminalScores(rowLines) {
  const rowText = normalizePdfLine(rowLines.join(' '));
  return /(?:^|\s)(?:-|\d+(?:\.\d+)?)(?:\s+(?:-|\d+(?:\.\d+)?)){6}$/.test(rowText);
}

function lbrceCredits({ L, T, P, C }) {
  const credits = {};
  if (L != null) credits.L = L;
  if (T != null) credits.T = T;
  if (P != null) credits.P = P;
  if (C != null) credits.C = C;
  return Object.keys(credits).length ? credits : null;
}

function lbrceExplicitCategoryFromLabel(label) {
  const text = normalizePdfLine(label);
  if (/^Program Elective/i.test(text)) return 'ProfessionalElective';
  if (/^Open Elective/i.test(text)) return 'OpenElective';
  return undefined;
}

function lbrceTitleAmbiguityReason({ title, label }) {
  const cleanTitle = cleanText(title);
  const cleanLabel = cleanText(label);
  if (/^Program Elective/i.test(cleanLabel) || /^Open Elective/i.test(cleanLabel)) {
    return 'elective row may represent an option group rather than one required subject';
  }
  if (/\bSWAYAM\b|\bNPTEL\b|\bMOOCs?\b/i.test(cleanTitle)) {
    return 'row references SWAYAM/NPTEL/MOOC choices rather than one clear subject title';
  }
  if (/\b(Student may select|specialized minors pool|honou?rs pool)\b/i.test(cleanTitle)) {
    return 'row describes a selectable pool rather than one clear subject';
  }
  return '';
}

function parseLbrceCourseRow({ rowLines, context, section, pageNumber }) {
  const rowText = normalizePdfLine(rowLines.join(' '));
  const score = '(-|\\d+(?:\\.\\d+)?)';
  const codedPattern = new RegExp(`^(\\d{1,2})\\s+((?=[A-Z0-9]*\\d)[A-Z0-9]{4,12})\\s+(.+?)\\s+${score}\\s+${score}\\s+${score}\\s+${score}\\s+${score}\\s+${score}\\s+${score}$`, 'i');
  const electivePattern = new RegExp(`^(\\d{1,2})\\s+((?:Program|Open)\\s+Elective-?\\s*[IVX]+)\\s*(.*?)\\s+${score}\\s+${score}\\s+${score}\\s+${score}\\s+${score}\\s+${score}\\s+${score}$`, 'i');
  const coded = rowText.match(codedPattern);
  const elective = coded ? null : rowText.match(electivePattern);
  const match = coded || elective;
  if (!match) {
    return {
      ignored: {
        page_number: pageNumber,
        reason: 'LBRCE row did not match a clear course-code/title/L-T-P/credits/evaluation pattern.',
        row_text: rowText,
      },
    };
  }
  ...
  const rowNumber = Number.parseInt(match[1], 10);
  const subjectCode = coded ? match[2] : null;
  const label = coded ? '' : cleanText(match[2]);
  const title = cleanText(coded ? match[3] : (match[3] || label));
  const offset = coded ? 4 : 4;
  const L = parsePdfNumber(match[offset]);
  const T = parsePdfNumber(match[offset + 1]);
  const P = parsePdfNumber(match[offset + 2]);
  const C = parsePdfNumber(match[offset + 3]);
  const category = lbrceExplicitCategoryFromLabel(label);
  const sectionType = section?.type_hint;
  const subjectType = typeFromLtpcAndTitle({ title, category, L, T, P }) || sectionType;
  const ambiguityReason = lbrceTitleAmbiguityReason({ title, label });
  const missing = [
    !context.regulation ? 'regulation' : '',
    !context.branch ? 'branch' : '',
    !context.year ? 'year' : '',
    !context.semester ? 'semester' : '',
    coded && !subjectCode ? 'course code' : '',
    !title ? 'title' : '',
    L == null || T == null || P == null || C == null ? 'L/T/P/credits' : '',
    ambiguityReason,
  ].filter(Boolean);
  const level = coded && missing.length === 0 ? 'high' : 'low';
    ...
    candidate: {
      ...
      evidence: {
        ...
        category_reason: category
          ? `${label} maps directly to ${category}.`
          : 'LBRCE course-structure row does not expose an explicit JNTUStack category column.',
      },
      confidence: {
        level,
        reason: level === 'high'
          ? 'LBRCE row has clear course code, title, L/T/P/credits, regulation, branch, and year/semester context; category still requires validation because the source table does not expose a direct category column.'
          : `LBRCE row needs review before extraction: ${missing.join(', ') || 'ambiguous source row'}`,
        signals: [
          ...
          'L/T/P/credits values present',
          category ? `explicit category ${category}` : 'no explicit category column in source row',
        ].filter(Boolean),
      },
    },
  };
...
function parseLbrceCourseRows({ pageTexts, asset, parserVersion }) {
  const fullText = pageTexts.join('\n\n');
  const globalContext = extractPdfContext({ asset, fullText });
  const candidates = [];
  const lowConfidenceCandidates = [];
  ...
  function finishRow() {
    if (!rowLines.length) return;
    pushParsed(parseLbrceCourseRow({
      rowLines,
      context: { ...globalContext, ...(activeSemester || {}) },
  ...
  pageTexts.forEach((pageText, pageIndex) => {
    ...
    for (const rawLine of pageText.split(/\n+/)) {
      ...
      if (!line) continue;
      headerLines.push(line);
      if (headerLines.length > 8) headerLines.shift();

      const semester = parseLbrceSemesterHeading(line);
      if (semester) {
        finishRow();
        activeSemester = semester;
        activeSection = null;
      ...
      if (isLbrceCourseStructureHeader(headerLines.join(' '))) {
        finishRow();
        inCourseTable = true;
        activeSection = null;
        headerLines = [];
      ...
      const section = lbrceSectionFromLine(line);
      if (section) {
        finishRow();
      ...
      if (
        rowLines.length
        && lbrceRowHasTerminalScores(rowLines)
        && (isLbrceCourseRowStart(line) || isLbrceStandaloneCourseCodeLine(line))
      ) {
        finishRow();
      }

      if (isLbrceCourseRowStart(line)) {
        if (rowLines.length) finishRow();
        rowLines = [line];
        rowPageNumber = pageNumber;
      } else if (rowLines.length) {
        rowLines.push(line);
  ...
  return {
    evidence_type: 'lbrce_r23_syllabus_pdf',
    evidence_status: 'needs_review',
    source_url: asset.sourceUrl,
    parser_version: parserVersion,
    page_count: pageTexts.length,
...
export function parseLbrceR23SyllabusTextForTest({ pageTexts, asset = {}, parserVersion = 'test' }) {
  return parseLbrceCourseRows({ pageTexts, asset, parserVersion });
}

async function parseLbrceR23SyllabusPdf({ asset, buffer, parserKey, parserVersion }) {
  const textResult = await extractPdfText({ asset, buffer, parserKey, parserVersion });
  const pageTexts = textResult.parsedPayload.page_texts || [];
  const parsedPayload = parseLbrceCourseRows({ pageTexts, asset, parserVersion });
  return {
    parsedPayload,
    confidence: {
      parser: parserKey,
      extraction: 'lbrce_r23_syllabus_pdf',
      page_count: parsedPayload.page_count,
      candidate_count: parsedPayload.candidates.length,
      low_confidence_candidate_count: parsedPayload.low_confidence_candidates.length,
      ignored_table_row_count: parsedPayload.ignored_table_rows.length,
      requires_human_review: true,
      no_auto_proposal: true,
      candidate_rule: 'Only coded rows from clear LBRCE course-structure tables are high-confidence candidates; category stays missing unless explicitly present in the source row.',
    },
  };
}
...
registerParser({
  key: 'lbrce-r23-syllabus-pdf',
  label: 'LBRCE R23 syllabus PDF',
  version: '0.1.0',
  description: 'Conservatively extracts subject candidates from LBRCE R23 syllabus PDF course-structure tables.',
  supports: ['application/pdf', '.pdf'],
  sourceSpecific: true,
  async parse({ asset, buffer }) {
    return parseLbrceR23SyllabusPdf({
      asset,
      buffer,
      parserKey: 'lbrce-r23-syllabus-pdf',
      parserVersion: '0.1.0',
    });
  },
});
