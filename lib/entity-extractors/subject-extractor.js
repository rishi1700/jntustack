...
function normalizeTitleWord(word, index) {
  ...
  if (/^[a-z]+$/.test(word)) return `${word[0].toUpperCase()}${word.slice(1).toLowerCase()}`;
  return word;
}
...
export function normalizeCourseTitle(title) {
  ...
  let index = 0;
  return value.replace(/[A-Za-z][A-Za-z0-9]*/g, word => normalizeTitleWord(word, index++));
}
...
export function extractSubjectPayload({ parsedPayload = {}, entityKey = '', hints = {}, candidateIndex = null }) {
  ...
  const subjectName = normalizeCourseTitle(firstText(candidateName, name));
