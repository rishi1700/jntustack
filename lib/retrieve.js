...
export function buildSearchIndex({ subjects = [], branchProfiles = [], colleges = [] }) {
  ...
  for (const s of subjects) {
    ...
    // A subject belongs to either one branch (s.branch) or several
    // (s.branchCodes, for content shared across branches at one URL) --
    // never both. Kept inline rather than imported from lib/dataset.js
    // (which pulls in node:fs) since this file loads unchanged in the browser.
    const branchLabel = s.branchCodes?.length ? s.branchCodes.join('/') : s.branch;
    const text = [
      s.name, branchLabel, s.year_sem_label, s.category,
    ...
    docs.push({
      ...
      id: s.id,
      url: `/${s.seo?.slug || s.id}/`,
      title: `${s.name} (${branchLabel} ${s.year_sem_label}, ${s.regulation})`,
      text,
      summary: {
        regulation: s.regulation, branch: branchLabel, year_sem: s.year_sem_label,
