...
export async function assertAssetStorageIntakeReady({
  root,
  env = process.env,
  persistenceCheck = checkAssetStoragePersistence,
} = {}) {
  let persistence = null;
  try {
    persistence = await persistenceCheck({ root, env, seedIfMissing: false });
  } catch {
    // Fail closed without surfacing filesystem or provider details.
  }
  if (persistence?.ready) return persistence;

  throw new AssetIntegrityError(
    'Evidence intake is disabled until durable asset storage has survived a commit-changing deployment.',
...
export async function registerAsset({
  ...
  await assertAssetStorageIntakeReady({
    root,
    env,
    persistenceCheck: storagePersistenceCheck,
  });
