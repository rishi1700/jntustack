...
async function inspectChecksumMatches(conn, checksum, root, { excludeAssetId = null } = {}) {
  ...
  for (const row of rows) {
    ...
    if (await assetFileExists(root, asset.localStoragePath)) {
      return { duplicate: asset, missing };
    }
    missing.push(asset);
  }
  return { duplicate: null, missing };
}
...
async function writeStoredFile({ root, sourceId, filename, checksum, buffer, now }) {
...
export async function registerAsset({
  ...
  try {
    ...
    const duplicate = checksumInspection.duplicate;
    const now = new Date();
    ...
    if (duplicate) {
      ...
      localStoragePath = await writeStoredFile({ root, sourceId, filename, checksum, buffer, now });
    }
...
export async function repairAssetRecordWithBuffer({
  ...
  try {
    ...
    const sourceId = normalizeId(before.discovery_source_id, 'Discovery source ID');
    const now = new Date();
    const checksumInspection = await inspectChecksumMatches(conn, checksum, root, { excludeAssetId: id });
    ...
    if (checksumInspection.duplicate) {
      ...
      duplicateOf = checksumInspection.duplicate;
      localStoragePath = duplicateOf.localStoragePath;
      downloadStatus = 'duplicate';
      status = 'ignored';
      duplicateOfAssetId = duplicateOf.duplicateOfAssetId || duplicateOf.id;
    } else {
      localStoragePath = await writeStoredFile({ root, sourceId, filename, checksum, buffer, now });
    }
    ...
    const previousState = {
      source_url: before.source_url ?? before.url,
      local_storage_path: before.local_storage_path ?? before.storage_path,
      download_status: before.download_status ?? before.status,
      status: before.status,
      sha256_checksum: before.sha256_checksum ?? before.checksum,
      file_size: before.file_size,
      content_type: before.content_type,
      downloaded_at: before.downloaded_at ?? before.fetched_at,
    };
    const metadata = mergeAssetMetadata(before.metadata_json, {
      file_repair: {
        status: 'repaired',
    ...
    await conn.execute(
      ...
      [
        filename,
        clean(assetKind) || before.asset_kind || 'manual_fetch',
        clean(contentType) || null,
        buffer.length,
        checksum,
        checksum,
...
export async function getAssetFileStatus(root, asset) {
  ...
  const exists = await assetFileExists(root, asset.localStoragePath);
  const repaired = Boolean(asset.metadataJson?.file_repair?.status === 'repaired');
  return {
    status: exists && repaired ? 'repaired' : exists ? 'present' : 'missing',
    exists,
    repaired,
    repairAvailable: !exists && Boolean(asset.sourceUrl && asset.discoverySourceId),
    checkedPath: asset.localStoragePath || null,
