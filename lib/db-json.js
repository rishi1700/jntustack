...
function normalizeImportOptions(options = {}) {
  ...
  if (file && [explicitScopes.subjects, explicitScopes.colleges, explicitScopes.branchProfiles].filter(Boolean).length > 1) {
    throw new Error('--file can only be combined with the matching scoped import flag.');
  }
  const hasScope = explicitScopes.subjects || explicitScopes.colleges || explicitScopes.branchProfiles;
  const full = !hasScope;
  return {
    file,
    full,
...
function importSummary(content, scopes) {
  ...
  return {
    ...
    regulations: scopes.subjects || scopes.branchProfiles || scopes.full ? content.shared.regulations.length : 0,
    branches: scopes.subjects || scopes.branchProfiles || scopes.full ? content.shared.branches.length : 0,
...
export async function importJsonContent(conn, dataDir, options = {}) {
  ...
  try {
    ...
    if (scopes.subjects || scopes.branchProfiles) {
