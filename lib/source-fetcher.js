    ...
    if (existing) {
      ...
      if (!(await assetFileExists(root, existingPath))) {
        await audit({
          ...
          after: { reason: 'stored_file_missing', source_url: initialUrl.href, storage_path: existingPath },
        });
        const repair = await repairExistingAssetFromUrl({
          ...
          actor,
          reason: 'source_url_file_missing',
        });
        await audit({
          actor,
...
export async function repairMissingAssetFile({
  ...
  const existingPath = asset.localStoragePath;
  if (await assetFileExists(root, existingPath)) {
  ...
  await audit({
    ...
    entityType: 'source_asset',
    entityId: asset.id,
    after: { reason: 'admin_repair_file_missing', source_url: asset.sourceUrl, storage_path: existingPath },
  });
  return repairExistingAssetFromUrl({
    ...
    actor,
    reason: 'admin_repair_file_missing',
  });
}
