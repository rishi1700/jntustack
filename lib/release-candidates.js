...
export async function markReleaseCandidateReady({ releaseCandidateId, actor = null }) {
  ...
  try {
    ...
    const release = releaseRows[0];
    if (!release) throw new Error(`Release candidate not found: ${releaseCandidateId}`);
    if (release.status !== 'draft') throw new Error('Only draft release candidates can be marked ready for review.');
    const [countRows] = await conn.execute(
      'SELECT COUNT(*) AS item_count FROM release_candidate_items WHERE release_candidate_id = ?',
      [releaseCandidateId]
    );
    if (Number(countRows[0]?.item_count || 0) === 0) {
      throw new Error('Add at least one approved proposal before marking a release candidate ready.');
    }
    await conn.execute(
