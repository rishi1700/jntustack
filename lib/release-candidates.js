...
import { getContentPublicationMode } from './config.js';
...
export function releaseArtifactsMutable(status) {
  return ['draft', 'applied_to_draft'].includes(status);
}
...
function releaseFromRow(row) {
  ...
  return {
    ...
    title: row.title,
    status: row.status,
    publicationMode: row.publication_mode || 'legacy',
...
export async function createReleaseCandidate({ title, actor = null }) {
  ...
  const publicationMode = getContentPublicationMode();
  const pool = await getDbPool({ requireConfigured: true });
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    const [result] = await conn.execute(
      `INSERT INTO release_candidates (title, status, publication_mode, created_by)
       VALUES (?, 'draft', ?, ?)`,
      [cleanTitle, publicationMode, actor]
    );
...
export async function exportReleaseCandidateItem({ root, releaseCandidateId, itemId, actor = null }) {
  const pool = await getDbPool({ requireConfigured: true });
  const [rows] = await pool.execute(
    `SELECT rci.*, cp.status AS proposal_status, rc.status AS release_status
     FROM release_candidate_items rci
     INNER JOIN content_proposals cp ON cp.id = rci.proposal_id
     INNER JOIN release_candidates rc ON rc.id = rci.release_candidate_id
     WHERE rci.id = ? AND rci.release_candidate_id = ?`,
    [itemId, releaseCandidateId]
  );
  const item = rows[0];
  if (!item) throw new Error(`Release candidate item not found: ${itemId}`);
  if (!releaseArtifactsMutable(item.release_status)) {
    throw new Error('Release artifacts cannot be changed after the release is marked ready for review.');
  }
  if (item.proposal_export_id) throw new Error('This release item already has an export. Open the existing export instead of creating another.');
...
export async function applyReleaseCandidateItemDraft({ root, releaseCandidateId, itemId, actor = null }) {
  ...
  if (item.draft_apply_id) throw new Error('This release item is already applied to the draft workspace. Open the existing draft instead of applying it again.');
  if (item.proposal_status !== 'approved_for_draft') {
