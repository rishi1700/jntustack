import { getAsset, readAssetBuffer } from './assets.js';
...
export async function runParser({
  ...
  try {
    ...
    const buffer = await readAssetBuffer({ root, asset, storage, env, fetchImpl });
    await conn.execute(
      'UPDATE source_assets SET storage_verified_at = CURRENT_TIMESTAMP WHERE id = ?',
      [asset.id]
    );
